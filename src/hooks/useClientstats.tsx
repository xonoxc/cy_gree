...
export const ClientStatsProvider = ({
    ...
    const collectionCreate = async ({
        ...
        if (!picture)
            throw new Error("Picture is required for creating collection")
    ...
    const handleCollectionCreate = useMutation({
        ...
        onMutate: async ({
            ...
            const previousData =
                queryClient.getQueryData<DefiniteIRequestsData>([
                    "client",
                    userId,
                    "history",
                ])

            queryClient.setQueryData<DefiniteIRequestsData>(
